...
class IMU_Serial:
    ...
    def _parse_data_frame(self, frame_type, frame_data):
        ...
        # 解析融合算法类型
        elif frame_type == self._CMD_ALGORITHM_TYPE:
            self._current_fusion_algorithm = struct.unpack('B', bytearray(frame_data[0:1]))[0]
    ...
    def get_fusion_algorithm(self):
        """
        获取当前融合算法类型
        
        返回:
            当前融合算法类型（6、9或10），获取失败返回None
        """
        try:
            # 首先尝试从内部状态变量获取
            if hasattr(self, '_current_fusion_algorithm'):
                return self._current_fusion_algorithm
            
            # 如果没有内部状态，尝试从硬件读取
            # 构建请求命令
            cmd = self._construct_request_command(self._CMD_ALGORITHM_TYPE)
            ...
            # 发送命令
            self._send_command(cmd)
            self._print_debug(cmd, "get algo type")
            ...
            while time.time() < timeout:
                # 检查是否有直接返回的算法类型帧
                if hasattr(self, '_current_fusion_algorithm'):
                    algo_type = self._current_fusion_algorithm
                    if algo_type in [6, 9, 10]:
                        return algo_type
                    break
                # 检查是否有返回状态帧
                elif self._rx_function == self._CMD_ALGORITHM_TYPE:
                    # 获取到了算法类型
                    algo_type = self._rx_state
                    if algo_type in [6, 9, 10]:
                        self._current_fusion_algorithm = algo_type
                        return algo_type
                    break
    ...
    def set_fusion_algorithm(self, algo_type):
        ...
        参数:
            algo_type: 算法类型，6表示六轴算法，9表示九轴算法，10表示十轴算法
        """
        if algo_type not in [6, 9, 10]:
            return  # 无效的算法类型
    ...
    def calibrate_imu(self):
        ...
        # 等待校准完成并返回结果
        return self._wait_for_calibration(self._CMD_CALIB_IMU, "cali imu", 7000)
    ...
    def calibrate_magnetometer(self):
        ...
        # 等待校准完成并返回结果
        return self._wait_for_calibration(self._CMD_CALIB_MAGNETOMETER, "cali mag")
...
if __name__ == '__main__':
    ...
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='IMU串口数据读取工具')
    parser.add_argument('--port', '-p', help='串口端口号，如/dev/ttyUSB0或COM3')
    parser.add_argument('--rate', '-r', type=int, default=10, help='数据报告频率(Hz)，默认10Hz')
    ...
    if args.port:
        # 使用用户指定的端口
        print(f"尝试打开串口: {args.port}")
        imu_serial = IMU_Serial(args.port, debug=args.debug)
        ...
        if system == 'Windows':
            ...
            while True:
                ...
                try:
                    ...
                    print(f"尝试打开串口: {port}")
                    imu_serial = IMU_Serial(port, debug=args.debug)
                    break
                except Exception as e:
                    if com_index > 256:
                        print("-----------------------未找到可用串口--------------------------")
            ...
            # Linux系统尝试常用串口（包含固定端口/dev/imu-serial）
            common_ports = ["/dev/imu-serial", "/dev/myserial", "/dev/ttyUSB0", "/dev/ttyUSB1", 
    ...
    # 设置数据报告频率
    imu_serial.set_report_rate(args.rate)
    print(f"数据报告频率已设置为: {args.rate} Hz")

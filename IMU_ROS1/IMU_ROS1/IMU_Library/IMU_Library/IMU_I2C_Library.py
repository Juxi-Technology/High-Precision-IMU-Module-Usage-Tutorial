...
class IMU_I2C:
    ...
    def __init__(self, port=1, debug=False):
        """
        初始化IMU I2C通信
    ...
    def get_fusion_algorithm(self):
        ...
        try:
            ...
            # 如果没有内部状态，尝试从硬件读取
            values = self._read_data(self.FUNC_ALGO_TYPE, 1)
    ...
    def set_fusion_algorithm(self, algo):
        ...
        参数:
            algo: 算法类型，6表示六轴算法，9表示九轴算法，10表示十轴算法
        """
        if algo not in [6, 9, 10]:
            return
        
        # 保存当前算法类型
        self._current_fusion_algorithm = algo
    ...
    def calibrate_imu(self):
        ...
        return self._wait_calibration(self.FUNC_CALIB_IMU, "cali imu", 7000)
    ...
    def calibrate_magnetometer(self):
        ...
        if self._delay_time > 0:
            time.sleep(self._delay_time)
        return self._wait_calibration(self.FUNC_CALIB_MAG, "cali mag")
    ...
    def calibrate_temperature(self, current_temperature):
        ...
        if self._delay_time > 0:
            time.sleep(self._delay_time)
        return self._wait_calibration(self.FUNC_CALIB_TEMP, "cali temp", 2000)
    ...
    def start_data_reception(self):
        """
        启动数据接收（I2C模式下为空实现，保持与串口库接口一致）
        
        I2C通信采用请求-响应模式，无需持续接收线程
        """
        if self._debug:
            print("-----------------I2C数据接收已准备-----------------")
